...
\maketitle
	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			...
			On sait qu'il existe $N \geqslant n_0$ tq $\sigma(N) < 0$, $\sigma(N+1) \geqslant 0$ (sinon, la suite resterait dans un des $E^-$ ou $E^+$ à partir d'un certain rang, et on a montré plus haut que c'était absurde). Dès lors, on a:
			$$\sum_{k=0}^{N} u_{\sigma(n)} \geqslant \alpha,\ \sum_{k=0}^{N+1} u_{\sigma(n)} < \alpha$$
			On déduit de la première que $\sum_{k=0}^{N+1} u_{\sigma(n)} \geqslant \alpha + \varepsilon$, d'où:
			$$\left| \sum_{k=0}^{N+1} u_{\sigma(n)} - \alpha \right| \leqslant \varepsilon$$
			\textbf{incomplet; je dois aller manger}
		\end{correctionbox}
