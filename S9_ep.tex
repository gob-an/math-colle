	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			\begin{itemize}
				...
				Si $\sum u_n$ diverge, on a le résultat en prenant $\sigma = \mathrm{Id}$. Supposons cette série convergente. On vérifie aisément que $\displaystyle \sum_{u_n \geqslant 0} u_n$ et $\displaystyle \sum_{u_n \leqslant 0} u_n$ divergent respectivement vers $+\infty$ et $-\infty$. \\
		...
		\begin{correctionbox}
			...
			On va procéder de même que précédemment:
			\begin{itemize}
				\item On ajoute des éléments positifs jusqu'à ce que la somme partielle dépasse $\alpha$.
				\item On ajoute ensuite des éléments négatifs jusqu'à ce que la somme partielle soit inférieure à $\alpha$. \\
			\end{itemize}
			On itère, et on montre de même qu'il s'agit bien d'une bijection. \\
			Soit $\varepsilon > 0$. Puisque $\sum u_n$ CV, il existe $n_0$ tq, pour $n \geqslant n_0$, $| \alpha | \leqslant \varepsilon$. Supposons, quitte à choisir $n_0$ assez grand, que $n_0$ arrive après un terme négatif. Si on note $n_1 = \sigma^{-1}(n_0)$, on montre aisément (oui j'ai la flemme de prouver... Je le démontre un jour) qu'à partir du rang $n_1$, la somme partielle est à une distance inférieure à $\varepsilon$ de $\alpha$. \\ \\
			D'où la convergence: $\overline {\mathbb R}$ tout entier est atteint. \\ \\
			Encore une promesse en l'air: un jour je rédige mieux.
		\end{correctionbox}
		\textit{Note}: Si vous voulez voir une jolie rédaction avant que je la fasse, cherchez "principe de réarrangement de Riemann" \\
		\item Pourquoi, en ce qui concerne les probabilités, l'ordre de sommation n'a pas d'importance?
		\begin{correctionbox}
			On a des éléments positifs, et la somme sur une partie finie est majorée par 1, d'où la sommabilité.
		\end{correctionbox}
