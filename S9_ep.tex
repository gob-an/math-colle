...
\maketitle
	...
	\subsection*{Exercice 17 \textnormal{(incomplet; question 4 non finie)}}
	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			Soit $\sigma \in S_n^j$. \\
			On considère l'ensemble des antécédents $I$ et l'image $k$ de $n$. \\
			On note $\sigma' : x \mapsto \begin{dcases}
				k & \text{si\ } x \in I \\
				\sigma(x) & \text{sinon}
			\end{dcases}$ \\
			Alors $\sigma$ est entièrement déterminé par la donnée de $\sigma'$ et de l'ensemble $I$ \\
			On vérifie aisément que $\sigma'$ est surjective dans $[\![ 1, n [\![$. \\
			\begin{itemize}
				\item Si $k \not \in I$, alors $\sigma'$ parcourt $S_{n-1}^j$.
				\item Sinon, il parcourt l'ensemble des éléments de $S_{n-1}^j$ laissant invariant $k$. \\
			\end{itemize}
			\textbf{FONCTIONNE PAS EN L'ÉTAT}
		\end{correctionbox}
