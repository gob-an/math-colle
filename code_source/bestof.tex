...
\begin{document}
	...
	\\ \\ \\
		...
		Pour $E$ euclidien, $S^+(E)$ engendre $L(E)$
	\end{thm}
	\begin{proof}
		La démonstration se fera matriciellement, pour plus de commodité. \\
		D'une part, on a $\Vect S_n^+(\R) = S_n(\R)$.
		De plus, on sait que $\dim \Vect S_n(\R)$ est de l'ordre de $n^2$, qui est bien la dimension de $M_n(\R)$. On a une inclusion évidente, d'où l'égalité.
	\end{proof}
	\begin{proof}
		Deux pour le prix d'une! \\
		On sait que les matrices diagonalisables sont denses dans $M_n(\R)$, et les matrices symétriques sont diagonalisables (théorème spectral): on en déduit que $S_n(\R)$ est dense dans $M_n(\R)$. \\
		Or, $S_n(\R)$ est un sev de dimension finie, donc est fermé: on en déduit que $S_n(\R) = \bar{S_n(\R)} = M_n(\R)$, d'où le résultat.
	\end{proof}
	\textbf{Corollaire} Toute matrice réelle est symétrique. En particulier, toute matrice réelle est diagonalisable (par théorème spectral). \\ \\
	Utilisé dans l'exercice 25 de la semaine 16: si un endo commute avec tous les autoadjoints positifs, alors c'est une homothétie (ce qui est vrai, mais pas pour cette raison)
\end{document}