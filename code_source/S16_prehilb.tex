...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			...
			On suppose qu'il existe un tel $A$. On va sortir de notre chapeau le $n$-ième polynôme de Tchebychev (on rappelle qu'il s'agit de l'unique polynôme $T_n$ tel que, pour tout $\theta$, $T_n(\cos \theta) = \cos n\theta$.) On a alors $T_n(1) = T_n(\cos 2\pi) = \cos 2n\pi = 1$, d'où:
			$$\int_0^1 \frac{T_n(t) A(t)}{\sqrt{1-t^2}}dt = 1$$
			Or, on a par changement de variable ($t = \cos \theta$), puis par IPP:
			\begin{align*}
				\int_0^1 \frac{T_n(t) A(t)}{\sqrt{1-t^2}}dt & = \int_0^\pi \frac{T_n(\cos\theta) A(\cos \theta)}{\sqrt{1-\cos^2\theta} \sqrt{1-\theta^2}}d\theta = \int_0^\pi \frac {\cos n \theta A \cos \theta} {\sqrt{1-\cos^2\theta} \sqrt{1-\theta^2}}d\theta \\
				& = \frac 1 n \left(\left[\frac {\sin n\theta A (\cos\theta)} {\sqrt{1-\cos^2\theta} \sqrt{1-\theta^2}} \right]_0^1 + \int_0^1 \sin n\theta f'(\theta) d\theta\right)
			\end{align*}
			(On vérifie par analyse asymptotique que le crochet converge bel et bien.)
			Or, les termes dans la parenthèse sont bornés, donc $T_n(1) \longrightarrow 0$, ce qui est absurde.
		\end{correctionbox}
	\end{enumerate}
