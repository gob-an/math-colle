...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			...
			De plus, $G'(1) = \displaystyle \sum_{k=2}^N \mu(k)$, qui est différent de 0 si $\mu(1) \not = 1$: 1 est alors racine simple de $1-G$, donc pôle simple de $F$. Si $\mu(1)=1$, $S_n=n$ presque sûrement, d'où $F(z)=\displaystyle \sum_{n\in\N*} z^n = \frac 1 {1-x}$ sur $]-1, 1[$, donc 1 est pôle simple. \\ \\
			De plus, si $z$ est pôle de $F$ de module 1, alors $\displaystyle \left|\sum_{k=1}^N \mu(k) z^k\right| \leqslant \sum_{k=1}^N |\mu(k) z^k| \leqslant \sum_{k=1}^N \mu(k) = 1$, et il y a égalité entre le premier et le dernier terme donc égalité à chaque inégalité. Ainsi, les $z^k$ sont positivement colinéaires donc $z\in \R_+$: on a bien $z=1$.
		\end{correctionbox}
	\end{enumerate}
