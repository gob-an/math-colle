...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			...
			On a donc $\pr(n\in E) = \displaystyle \sum_{k=1}^N \mu(k) \pr(n-k \in E') = \sum_{k=1}^N \mu(k) \pr\left(n-ke \in E\right)$			
		\end{correctionbox}
		...
		\begin{correctionbox}
			...
			Or, $G(1)=\displaystyle \sum_{k=1}^N \mu(k) = 1$: 1 est pôle de F (je ne sais pas montrer qu'il est simple). \\ \\
			De plus, en notant $Y = \min E$, on sait que $\pr(n \in E) \leqslant \pr(Y\leqslant n) \leqslant \pr(Y=n)$. Dès lors, $F$ est de rayon supérieur à celui de la fonction génératrice de $Y$, qui est au moins 1. (il est d'ailleurs égal à 1 d'après la première partie de cette question.) \\ \\
			On sait donc que si $z$ est pôle de $F$, $|z|\geqslant 1$. Supposons $z$ pôle de $F$ de module 1.
			\\ 
			Alors $\displaystyle \left|\sum_{k=1}^N \mu(k) z^k\right| \leqslant \sum_{k=1}^N |\mu(k) z^k| = \sum_{k=1}^N \mu(k) = 1$, et il y a égalité entre le premier et le dernier terme donc égalité à chaque inégalité. Ainsi, les $z^k$ sont positivement colinéaires donc $z\in \R_+$: on a bien $z=1$.
		\end{correctionbox}
	\end{enumerate}
