...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			...
			\begin{align*}
				F(z) & = \sum_{n\in\N} \pr(n\in E) z^n = \sum_{n\in\N^*} z^n \sum_{k=1}^N \mu(k) \pr(n-k \in E) + \pr(0\in E) \\
				& = \sum_{i\in \N} \sum_{n-k=i} \mu(k) \pr(i\in E) z^n + 1 \	\ \text{(tout est sommable dans le rayon de convergence)} \\
				& = \sum_{i\in \N} \sum_{k=1}^N z^i z^k \mu(k) \pr(i\in E) + 1 \	\ \text{(les termes pour $k>N$ étant nuls)} \\
				& = \sum_{i\in\N} z^i \pr(i\in E) \sum_{k=1}^N \mu(k) z^k + 1 = G(z) \sum_{i\in\N} z^i \pr(i\in E) + 1 \\
				& = G(z) F(z) + 1 \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \text{(car $\pr(0 \in E)=1$: $S_0=0$)} %c'est très moche...
			\end{align*}
			On en déduit $F(z) = \frac 1 {1-G(z)}$.
		\end{correctionbox}
