...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{correctionbox}
			...
			Calculons, par récurrence sur $n$ (eww), $\displaystyle I_n = \int_{0}^{+\infty} x^n e^{-x}$. \\
			On a, pour $n>0$ $\displaystyle I_n = \int_{0}^{+\infty} x^n e^{-x} = -[x^n e^{-x}]_0^{+\infty} + \int_{0}^{+\infty} x^{n-1} e^{-x} = 0 + I_{n-1}$ (le crochet vaut 0 parce qu'on a supposé $n \not = 0$, il faut faire attention!) Le crochet converge, et $I_{n-1}$ converge par HR, d'où la convergence de $I_n$, et l'égalité. \\
			On en déduit $I_n = I_0 = -1$. \\ \\
			Dès lors, l'intégrale de l'énoncé vaut $\displaystyle -\sum_{n\in I} \frac 1 {n!}$. \\
			\textbf{il y a une erreur, je connais pas beaucoup de fonctions positives d'intégrale négative}
		\end{correctionbox}
	\end{enumerate}
